...
def load_info(data_path):
    ...
    for task in ['train', 'dev', 'test']:
        data = pickle.load(open('data/phoenix-2014/{:s}.pkl'.format(task), 'rb'))
        task_info = []
...
def parse_args():
    ...
    p.add_argument('-s', '--source_path', type=str, default='', help='source path of original data [trackedRightHand-92x132px]')
